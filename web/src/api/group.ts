...
export interface Group {
  ...
  // 自动匹配规则（Go 正则，匹配模型名）；空串表示未启用。
  // 保存与渠道变更时服务端按它补齐可用授权，只追加不删除。
...
export interface GroupCreateRequest {
  ...
  mode: GroupMode;
  relay_config: GroupRelayConfig;
  auto_add_pattern: string; // 自动匹配规则；允许仅规则无手动成员，服务端保存时补齐匹配项。
...
export interface GroupUpdateRequest {
  ...
  mode?: GroupMode;
  relay_config?: GroupRelayConfig;
  auto_add_pattern?: string; // 空串表示清空规则：停止后续自动补齐，已有成员保留。
  items?: GroupItemInput[];
...
function writeGroupCache(group: Group) {
  queryClient.setQueryData(
    groupListQueryOptions.queryKey,
...
function removeGroupCache(id: number) {
  queryClient.setQueryData(
    groupListQueryOptions.queryKey,
    (current: Group[] | undefined) => current?.filter((item) => item.id !== id),
  );
...
function useGroupEventStream(enabled: boolean) {
  useEffect(() => {
    ...
    groupEventRefCount++;
    if (!groupEventSource) {
      const source = new EventSource("/api/v1/group/events", {
        withCredentials: true,
      });
      groupEventSource = source;
      source.addEventListener("changed", (event) => {
        writeGroupCache(
          JSON.parse((event as MessageEvent<string>).data) as Group,
        );
      });
      ...
      source.addEventListener("runtime", (event) => {
        ...
        queryClient.setQueryData(
          ...
          (current: Group[] | undefined) =>
            current?.map((group) =>
              group.id === update.group_id
                ? { ...group, runtime: update }
                : group,
            ),
        );
        queryClient.setQueryData(
          ["groups", "detail", update.group_id],
          (current: Group | undefined) =>
...
export function useUpdateGroup() {
  return useMutation({
    mutationFn: ({ id, ...data }: GroupUpdateRequest & { id: number }) =>
      apiRequest<Group>(`/api/v1/group/update/${id}`, {
        method: "POST",
        body: data,
      }),
    onSuccess: writeGroupCache,
...
export function useSetGroupItemEnabled() {
  return useMutation({
    mutationFn: (data: {
      ...
      apiRequest<Group>("/api/v1/group/item/enabled", {
        method: "POST",
        body: data,
      }),
    onSuccess: writeGroupCache,
    onError: (_error, variables) => {
      ...
      void queryClient.invalidateQueries({
        queryKey: ["groups", "detail", variables.group_id],
      });
    },
  });
}
