...
export function fromChannel(channel: ChannelDetail): ChannelFormState {
  return {
    ...
    keys: channel.keys.map(({ name, key, enabled }) => ({
      name,
      key,
      enabled,
    })),
    ...
    grants: new Map(
      channel.grants.map((g) => [
        grantKey(g.model_name, g.key_name),
        g.protocols,
      ]),
    ),
...
export function toChannelConfig(state: ChannelFormState) {
  return {
    ...
    custom_header: state.custom_header.filter(
      (h) => h.header_key.trim() && h.header_value !== "",
    ),
...
// 协议位为空的条目不是授权, 在此丢弃。
// revision 是打开草稿时捕获的版本令牌, 与表单状态分开传入: 后台刷新轮换 revision 不应改动本草稿,
// 也不允许表单内容反过来污染它; 创建时传空串由服务端生成。
export function toChannelDetail(
  ...
  return {
    ...
    id,
    revision,
    keys: state.keys.map(({ name, key, enabled }) => ({
      name: name.trim(),
      key: key.trim(),
      enabled,
    })),
